...
public class Tests {
	...
	public void ejecutarTestsAnularBilletes() {
		...
		try {
			servicio.anularBillete(null, null, null, null, 0, 7);
		...
		// Prueba caso se elimina el ticket
		try {
			servicio.anularBillete(null, null, null, null, 0, 2);
			PoolDeConexiones pool = PoolDeConexiones.getInstance();
			Connection con = pool.getConnection();
			PreparedStatement st1 = con.prepareStatement("select * from tickets where idTicket = 3");
			ResultSet sol = st1.executeQuery();
			if(sol.next()) {
				LOGGER.info("Eliminar ticket MAL");
			}else {
				LOGGER.info("Eliminar ticket OK");
			}
		} catch (SQLException e) {
